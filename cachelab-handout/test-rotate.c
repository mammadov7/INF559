/*
 * test-rotate.c - Checks the correctness and performance of all of the
 *     student's rotate functions and records the results for their
...
/* The description string for the rotate_submit() function that the
   student submits for credit */
#define SUBMIT_DESCRIPTION "Rotate submission"

/* External function defined in rotate.c */
...
/* The correctness and performance for the submitted rotate function */
struct results {
    int funcid;
    int correct;
    int misses;
...
/* 
 * eval_perf - Evaluate the performance of the registered rotate functions
 */
void eval_perf(unsigned int s, unsigned int E, unsigned int b)
{
    ...
    /* Evaluate the performance of each registered rotate function */
    ...
    for (i=0; i<func_counter; i++) {
        ...
        sprintf(cmd, "valgrind --tool=lackey --trace-mem=yes --log-fd=1 -v ./tracegen -N %d -F %d  > trace.tmp", N,i);
        flag=WEXITSTATUS(system(cmd));
        if (0!=flag) {
            printf("Validation error at function %d! Run ./tracegen -N %d -F %d for details.\nSkipping performance evaluation for this function.\n",flag-1,N,i);      
            continue;
        }
        ...
        /* Save the correctness of the rotate submission */
        if (results.funcid == i ) {
            results.correct = 1;
        }
        ...
        /* Filtered trace for each rotate function goes in a separate file */
        sprintf(filename, "trace.f%d", i);
        part_trace_fp = fopen(filename, "w");
        assert(part_trace_fp);
    
        /* Locate trace corresponding to the rotate function */
        flag = 0;
        ...
        /* If it is rotate_submit(), record number of misses */
        if (results.funcid == i) {
...
void sigsegv_handler(int signum){
    ...
    printf("TEST_ROTATE_RESULTS=0:0\n");
    fflush(stdout);
    exit(1);
}
